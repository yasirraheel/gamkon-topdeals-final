    ...
    <div class="container">
        <div class="section-title">
            ...
            <div class="right wow img-custom-anim-right" data-wow-duration="1.5s" data-wow-delay="0.2s">
                <a href="{{ route('all.listing', ['sort' => 'newest']) }}" class="view-all">{{ __('View All') }}</a>
            </div>

        </div>
        <div class="title_mt">
            <div class="row g-4 category">
                @forelse ($latestItemListing ?? [] as $listingIndex => $listing)
                    @include('frontend::listings.include.category-block', [
                        'listing' => $listing,
                        'isLatest' => true,
                    ])
                @empty
                    <x-luminous.no-data-found type="{{ __('Latest Items') }}" />
                @endforelse
            </div>
        </div>
    </div>
