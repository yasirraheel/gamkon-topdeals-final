                ...
                @if ($field['type'] == 'file')
                    <div class="site-input-groups row">
                        <div class="col-xl-4 col-lg-4 col-md-3 col-12 col-label">
                            ...
                            @if ($field['name'] == 'site_logo' || $field['name'] == 'site_logo_dark')
                                <i data-lucide="info" data-bs-toggle="tooltip" title=""
                                    data-bs-original-title="Recommended size: 100X42px"></i>
                            @elseif($field['name'] == 'site_favicon')
                                <i data-lucide="info" data-bs-toggle="tooltip" title=""
                                    data-bs-original-title="Recommended size: Square"></i>
                            @elseif($field['name'] == 'login_bg')
                                <i data-lucide="info" data-bs-toggle="tooltip" title=""
                                    data-bs-original-title="Recommended size: 1197X697px"></i>
                            @endif
                        </div>
                        <div class="col-xl-8 col-lg-8 col-md-9 col-12">
                            <div class="wrap-custom-file {{ $errors->has($field['name']) ? 'has-error' : '' }}">
                                <input type="{{ $field['type'] }}" name="{{ $field['name'] }}" id="{{ $field['name'] }}"
                                    value="{{ oldSetting($field['name'], $section) }}" accept=".jpeg, .jpg, .png,.svg" />
                                <label for="{{ __($field['name']) }}"
                                    @if (file_exists(base_path('assets/' . oldSetting($field['name'], $section)))) class="file-ok"
                    ...
                    <div class="site-input-groups row">
                        <label for="" class="col-sm-4 col-label">
                            ...
                            @if ($field['name'] == 'referral_code_limit')
                                <i data-lucide="info" data-bs-toggle="tooltip" title=""
                                    data-bs-original-title="This is the referral url code number count"></i>
                            @elseif($field['name'] == 'primary_color')
                                <i data-lucide="info" data-bs-toggle="tooltip" title=""
                                    data-bs-original-title="Define primary color for applications frontend including landing page, and user panel"></i>
                            @endif
                        </label>
                        <div class="col-sm-8">
