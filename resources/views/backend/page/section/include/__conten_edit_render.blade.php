        ...
        @foreach ($languages as $language)
            <li class="nav-item" role="presentation">
                <a href="" class="nav-link  {{ $loop->index == 0 ? 'active' : '' }}" id="pills-render-tab"
                    ...
                    aria-controls="pills-render" aria-selected="true"><i
                    ...
                    <form action="{{ route('admin.page.content-update') }}" method="post"
                        ...
                        @if ($key == 'en')
                            <div class="site-input-groups">
                                @if ($landingContent['type'] == 'whychooseus' || $landingContent['type'] == 'about')
                                    <label for="" class="box-input-label">{{ __('Icon Name') }} <a
                                    ...
                                    <input type="text" name="icon" class="box-input mb-0 icon"
                                        value="{{ $landingContent['icon'] }}" placeholder="Icon Class"
                                        required="" />
                                @elseif($landingContent['type'] == 'bankingsolution' || $landingContent['type'] == 'experiencesection')
                                    <label for="" class="box-input-label">{{ __('Icon Name') }} <a
                                            class="link" href="https://lucide.dev/icons/"
                                            target="_blank">{{ __('Lucide Icons') }}</a>:</label>
                                    <input type="text" name="icon" class="box-input mb-0" placeholder="Icon Name"
                                        value="{{ $landingContent['icon'] }}" required="" />
                                @elseif($landingContent['type'] == 'about-stats' || $landingContent['type'] == 'features')
                        ...
                        @if ($landingContent['type'] != 'about' && $landingContent['type'] != 'funfact')
                            <div class="site-input-groups mb-0">
                                ...
                                <textarea name="description" class="form-textarea description" placeholder="{{ __('Description') }}">{{ $landingContent['description'] }}</textarea>
                            </div>
                        @endif
