<form action="{{ route('admin.navigation.menu.update') }}" method="post">
    ...
    <div class="site-input-groups">
        ...
        <select name="type[]" multiple class="form-select type displayInSelect">
            ...
            <option value="{{ \App\Enums\NavigationType::FooterWidget2->value }}" @selected(in_array(\App\Enums\NavigationType::FooterWidget2->value, $navigation->type, true))>{{ __('Footer Widget 2') }}</option>
            <option value="{{ \App\Enums\NavigationType::FooterWidget3->value }}" @selected(in_array(\App\Enums\NavigationType::FooterWidget3->value, $navigation->type, true))>{{ __('Footer Widget 3') }}</option>
            <option value="{{ \App\Enums\NavigationType::FooterWidgetBottom->value }}" @selected(in_array(\App\Enums\NavigationType::FooterWidgetBottom->value, $navigation->type, true))>{{ __('Footer Bottom Widget') }}</option>
        </select>
    </div>
