...
class DepositController extends Controller
{
    ...
    public function pending(Request $request)
    {
        ...
        $deposits = Transaction::with('user')
            ...
            ->whereIn('type', [TxnType::Deposit->value, TxnType::PlanPurchased->value, TxnType::ProductOrder->value, TxnType::Topup->value])
            ->search($search)
    ...
    public function history(Request $request)
    {
        ...
        $deposits = Transaction::with('user')
            ->whereIn('type', [TxnType::ManualDeposit->value, TxnType::Deposit->value, TxnType::PlanPurchased->value, TxnType::Topup->value, TxnType::ProductOrder->value])
            ->search($search)
            ->when(in_array(request('sort_field'), ['created_at', 'amount', 'charge', 'method', 'status', 'tnx']), function ($query) {
                $query->orderBy(request('sort_field'), request('sort_dir'));
            })
    ...
    public function actionNow(Request $request)
    {
        ...
        if (isset($input['approve'])) {
            ...
            } elseif ($transaction->type == TxnType::ProductOrder) {
                orderService()->orderPaymentSuccess($transaction->order);
            } elseif ($transaction->type == TxnType::Topup) {
                $transaction->user->increment('topup_balance', $transaction->final_amount - $transaction->charge);
            } else {
                // Level referral
        ...
        $this->mailNotify($transaction->user->email, 'user_manual_payment_request', $shortcodes);
        $this->pushNotify('user_manual_payment_request', $shortcodes, route('user.transactions'), $transaction->user->id);
        $this->smsNotify('user_manual_payment_request', $shortcodes, $transaction->user->phone);
